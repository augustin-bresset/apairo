...
class Rellis3DDataset(SynchronousDataset):
    available_keys = frozenset(_AVAILABLE_KEYS)
    r"""Rellis-3D synchronous dataset (LiDAR + semantic labels).

    Args:
        root_dir: Dataset root containing a ``Rellis-3D/`` subdirectory.
    ...
    def __init__(self, root_dir: str | Path, keys: list[str] | None = None) -> None:
        ...
        if "lidar" in self._keys:
            self._files["lidar"] = sorted(
                root.glob("Rellis-3D/**/os1_cloud_node_kitti_bin/*.bin")
            )
