...
def get_indexes(timestamps: dict, reference=None) -> dict:
    ...
    indexes = {
        key: np.searchsorted(value, timestamps[reference])
...
def show_timestamps(timestamps: dict, index_max=None):
    ...
    for i, (key, value) in enumerate(timestamps.items()):
        ...
        plt.plot(
            value, i * np.ones_like(value), label=key, marker="o", linestyle="None"
        )
    plt.legend()
    plt.show()
...
def show_indexed_timestamps(timestamps: dict, indexes: dict, index_max=None):
    ...
    timestamps_indexed = {
        key: [
            value[indexes[key][i - 1] : indexes[key][i]]
            for i in range(1, len(indexes[key]))
        ]
        for key, value in timestamps.items()
    }
